...
public class HrmAchievementEmployeeAppraisalServiceImpl extends BaseServiceImpl<HrmAchievementEmployeeAppraisalMapper, HrmAchievementEmployeeAppraisal> implements IHrmAchievementEmployeeAppraisalService {
    ...
    public Map<Integer, Integer> queryAppraisalNum() {
        ...
        Integer myAppraisalCount = lambdaQuery().eq(HrmAchievementEmployeeAppraisal::getEmployeeId, EmployeeHolder.getEmployeeId()).count().intValue();
        //我的绩效
        ...
        Integer targetConfirmCount = lambdaQuery().eq(HrmAchievementEmployeeAppraisal::getFollowUpEmployeeId, EmployeeHolder.getEmployeeId())
                .eq(HrmAchievementEmployeeAppraisal::getStatus, EmployeeAppraisalStatus.PENDING_CONFIRMATION.getValue()).count().intValue();
        //目标确认
        ...
        Integer resultConfirmorsCount = employeeResultConfirmorsService.lambdaQuery().eq(HrmAchievementEmployeeResultConfirmors::getEmployeeId, EmployeeHolder.getEmployeeId()).eq(HrmAchievementEmployeeResultConfirmors::getStatus, 0).count().intValue();
        //结果确认
        map.put(4, resultConfirmorsCount);
        Integer myUnDoAppraisalCount = lambdaQuery().eq(HrmAchievementEmployeeAppraisal::getEmployeeId, EmployeeHolder.getEmployeeId()).eq(HrmAchievementEmployeeAppraisal::getStatus, EmployeeAppraisalStatus.TO_BE_FILLED.getValue()).count().intValue();
        //待填写绩效
        map.put(7, myUnDoAppraisalCount);
        Integer myDoAppraisalCount = lambdaQuery().eq(HrmAchievementEmployeeAppraisal::getEmployeeId, EmployeeHolder.getEmployeeId()).ne(HrmAchievementEmployeeAppraisal::getStatus, EmployeeAppraisalStatus.TO_BE_FILLED.getValue()).count().intValue();
        //已填写绩效
    ...
    @Override
    public EmployeeAppraisalDetailVO queryEmployeeAppraisalDetail(Long employeeAppraisalId) {
        ...
        Long tableId = appraisal.getTableId();
        ...
        Integer count = employeeEvaluatoService.lambdaQuery().eq(HrmAchievementEmployeeEvaluato::getEmployeeAppraisalId, employeeAppraisalId)
                .eq(HrmAchievementEmployeeEvaluato::getStatus, IsEnum.YES.getValue()).count().intValue();
        int isWrite = 0;
        if (employeeAppraisal.getStatus() < TWO || (employeeAppraisal.getStatus() == TWO && count == 0)) {
            isWrite = 1;
        }
    ...
    public void writeAppraisal(WriteAppraisalBO writeAppraisalBO) {
        ...
        Long employeeAppraisalId = writeAppraisalBO.getEmployeeAppraisalId();
        //删除之前写的
        List<Long> oldSegIds = employeeSegService.lambdaQuery().select(HrmAchievementEmployeeSeg::getSegId)
    ...
    public void addTargetConfirm(HrmAchievementEmployeeAppraisal employeeAppraisal) {
        ...
        if (targetConfirmorsOpt.isPresent()) {
            ...
            //如果下一确认人不为空,添加确认记录
            Long employeeId = achievementUtil.findEmployeeIdByType(targetConfirmors.getType(), employeeAppraisal.getEmployeeId(), targetConfirmors.getEmployeeId(), "目标确认");
            ...
            if (appraisal.getStatus() >= AppraisalStatus.UNDER_EVALUATION.getValue()) {
                ...
                updateById(employeeAppraisal);
                Integer unAssessedCount = lambdaQuery().eq(HrmAchievementEmployeeAppraisal::getStatus, EmployeeAppraisalStatus.TO_BE_ASSESSED.getValue()).eq(HrmAchievementEmployeeAppraisal::getAppraisalId, employeeAppraisal.getAppraisalId()).count().intValue();
                Integer count = lambdaQuery().eq(HrmAchievementEmployeeAppraisal::getAppraisalId, employeeAppraisal.getAppraisalId()).count().intValue();
                //未评定的数量等于总数量时
    ...
    private void addEvaluato(HrmAchievementEmployeeAppraisal employeeAppraisal, Integer type) {
        ...
        Optional<HrmAchievementEmployeeEvaluato> lastEmployeeEvaluatoOpt = employeeEvaluatoService.lambdaQuery()
                ...
                .orderByDesc(HrmAchievementEmployeeEvaluato::getSort).oneOpt();
        ...
        if (appraisalEvaluatorsOpt.isPresent()) {
            ...
            Long evaluatoEmployeeId = achievementUtil.findEmployeeIdByType(evaluators.getType(), employeeAppraisal.getEmployeeId(), evaluators.getEmployeeId(), "结果评定");
            ...
            Integer count = lambdaQuery().eq(HrmAchievementEmployeeAppraisal::getAppraisalId, appraisal.getAppraisalId()).count().intValue();
            //没有下一平定人,把考核状态修改为待结果确认,如果后天开启的结果确认,则需要计算评分
            employeeAppraisal.setStatus(EmployeeAppraisalStatus.TO_BE_CONFIRMED.getValue());
            updateById(employeeAppraisal);
            Integer unConfirmedCount = lambdaQuery().eq(HrmAchievementEmployeeAppraisal::getStatus, EmployeeAppraisalStatus.TO_BE_CONFIRMED.getValue()).eq(HrmAchievementEmployeeAppraisal::getAppraisalId, appraisal.getAppraisalId()).count().intValue();
            if (appraisal.getStatus() == AppraisalStatus.CONFIRMING.getValue()) {
                ...
                if (count.equals(unConfirmedCount)) {
                    List<Long> resultConfirmorsList = appraisalResultConfirmorsService.lambdaQuery().eq(HrmAchievementAppraisalResultConfirmors::getAppraisalId, employeeAppraisal.getAppraisalId())
                            .list().stream().map(HrmAchievementAppraisalResultConfirmors::getEmployeeId).collect(Collectors.toList());
                    //生成考评结果确认
                    resultConfirmorsList.forEach(employeeId -> {
                        ...
                        if (null == esultConfirmors) {
                            employeeResultConfirmors.setEmployeeId(Convert.toLong(employeeId));
                        ...
                        //发送目标结果待确认通知
                        sendMessage(UserUtil.getUserId(), Convert.toLong(employeeId), appraisal.getAppraisalId(), AdminMessageEnum.HRM_EMPLOYEE_APPRAISAL_CONFIRMED.getType(), appraisal.getAppraisalName());
                    });
                }
            }
    ...
    public void resultEvaluato(ResultEvaluatoBO resultEvaluatoBO) {
        ...
        HrmAchievementEmployeeEvaluato employeeEvaluato = employeeEvaluatoService.lambdaQuery()
                ...
                .orderByDesc(HrmAchievementEmployeeEvaluato::getSort).one();
        int three = 3;
        ...
        } else if (resultEvaluatoBO.getStatus() == TWO) {
            rejectTarget(employeeAppraisal, resultEvaluatoBO.getRejectReason());
        } else if (resultEvaluatoBO.getStatus() == three) {
            HrmAchievementEmployeeEvaluato lastEmployeeEvaluato = employeeEvaluatoService.lambdaQuery()
                    .eq(HrmAchievementEmployeeEvaluato::getEmployeeAppraisalId, employeeAppraisal.getEmployeeAppraisalId())
                    .eq(HrmAchievementEmployeeEvaluato::getSort, employeeEvaluato.getSort() - 1).one();
            if (lastEmployeeEvaluato != null) {
                //如果上一评定人不为空，则发送通知
                sendContentMessage(EmployeeCacheUtil.getUserId(employeeAppraisal.getEmployeeId()), lastEmployeeEvaluato.getEmployeeId(), resultEvaluatoBO.getEmployeeAppraisalId(), AdminMessageEnum.HRM_EMPLOYEE_APPRAISAL_ASSESSED_REJECT.getType(), hrmAchievementAppraisal.getAppraisalName(), UserUtil.getUser().getNickname());
            }
    ...
    private void rejectTarget(HrmAchievementEmployeeAppraisal employeeAppraisal, String reason) {
        ...
        //驳回目标后，考核员工收到通知
        sendContentMessage(UserUtil.getUserId(), employeeAppraisal.getEmployeeId(), employeeAppraisal.getEmployeeAppraisalId(), AdminMessageEnum.HRM_EMPLOYEE_APPRAISAL_WRITE_REJECT.getType(), appraisal.getAppraisalName(), UserUtil.getUser().getNickname());
    ...
    public BasePage<ResultConfirmListVO> queryResultConfirmList(BasePageBO basePageBO) {
        ...
        page.getList().forEach(resultConfirmListVO -> {
            ...
            mapList.forEach(map -> {
                scoreLevels.add(BeanUtil.toBean(map, ResultConfirmListVO.ScoreLevelsBean.class));
            });
    ...
    @Override
    public ResultConfirmByAppraisalIdVO queryResultConfirmByAppraisalId(Long appraisalId) {
        ...
        scoreLevelList.forEach(scoreLevel -> {
            ...
            mapList.forEach(map -> {
                employeesBeanList.add(BeanUtil.toBean(map, ResultConfirmByAppraisalIdVO.EmployeesBean.class));
            });
    ...
    @Override
    public void resultConfirm(Long appraisalId) {
        ...
        employeeResultConfirmorsService.lambdaUpdate().set(HrmAchievementEmployeeResultConfirmors::getStatus, 1)
                ...
                .update();
        boolean exists = employeeResultConfirmorsService.lambdaQuery()
                .eq(HrmAchievementEmployeeResultConfirmors::getStatus, 0)
                .eq(HrmAchievementEmployeeResultConfirmors::getAppraisalId, appraisalId).exists();
        List<Long> employeeAppraisalIds = lambdaQuery().select(HrmAchievementEmployeeAppraisal::getEmployeeAppraisalId)
        ...
        if (!exists) {
            ...
            sendSponsorMessage(UserUtil.getUserId(), appraisal.getCreateUserId(), appraisalId, AdminMessageEnum.HRM_APPRAISAL_ARCHIVE.getType(), appraisal.getAppraisalName());
            //被考核人收到考核完成通知
            List<Long> employeeIds = employeeResultConfirmorsService.lambdaQuery()
        ...
        employeeAppraisalActionRecordService.confirmResult(appraisal.getAppraisalName(), employeeAppraisalIds, !exists);
    }
    ...
    @Override
    public List<AchievementAppraisalVO> queryTargetConfirmScreen(Long employeeId, Integer status) {
        ...
        if (targetConfirmListVOList.size() > 0) {
            List<Long> collect = targetConfirmListVOList.stream().map(TargetConfirmListVO::getAppraisalId).distinct().collect(Collectors.toList());
    ...
    @Override
    public List<AchievementAppraisalVO> queryEvaluatoScreen(Long employeeId, Integer status) {
        ...
        if (evaluatoListVOBaseList.size() > 0) {
            List<Long> collect = evaluatoListVOBaseList.stream().map(EvaluatoListVO::getAppraisalId).distinct().collect(Collectors.toList());
            List<HrmAchievementAppraisal> list = appraisalService.query().select("appraisal_id", "appraisal_name").in("appraisal_id", collect).list();
            return list.stream().map(data -> BeanUtil.copyProperties(data, AchievementAppraisalVO.class)).collect(Collectors.toList());
        }
        return Collections.emptyList();
    ...
    @Override
    public Long queryLevelIdByScore(QueryLevelIdByScoreBO queryLevelIdByScoreBO) {
        ...
        //查询出对应的等级id
        Long levelId = scoreRangeMap.get(queryLevelIdByScoreBO.getScore());
        return levelId;
    }
     ...
     * @param content    内容
     */
    public void sendContentMessage(Long userId, Long employeeId, Long typeId, Integer type, String title, String content) {
        AdminMessage adminMessage = new AdminMessage();
        adminMessage.setCreateUser(userId);
        adminMessage.setCreateTime(LocalDateTime.now());
        adminMessage.setRecipientUser(EmployeeCacheUtil.getUserId(employeeId));
     ...
     * @param title
     */
    public void sendSponsorMessage(Long userId, Long recipientUserId, Long typeId, Integer type, String title) {
        AdminMessage adminMessage = new AdminMessage();
        adminMessage.setCreateUser(userId);
        adminMessage.setCreateTime(LocalDateTime.now());

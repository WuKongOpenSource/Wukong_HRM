...
public class HrmSalaryArchivesServiceImpl extends BaseServiceImpl<HrmSalaryArchivesMapper, HrmSalaryArchives> implements IHrmSalaryArchivesService {
    ...
    public OperationLog setFixSalaryRecord(SetFixSalaryRecordBO setFixSalaryRecordBO) {
        ...
        operationLog.setOperationInfo(StrUtil.format("定薪原因：{},生效时间：{},生效状态：{},调整前工资：{},调整后工资：{}", SalaryChangeReasonEnum.parseName(salaryChangeRecord.getChangeReason()), salaryChangeRecord.getEnableDate().toString(), SalaryChangeRecordStatusEnum.HAS_TAKE_EFFECT.getName(), ObjectUtil.defaultIfBlank(salaryChangeRecord.getBeforeTotal(), "0"), salaryChangeRecord.getAfterTotal()));
        return operationLog;
    }
    ...
    @Override
    public FixSalaryRecordDetailVO queryFixSalaryRecordById(Long id) {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public OperationLog setChangeSalaryRecord(SetChangeSalaryRecordBO setChangeSalaryRecordBO) {
        ...
        operationLog.setOperationInfo(StrUtil.format("调薪原因：{},生效时间：{},生效状态：{},调整前工资：{},调整后工资：{}", SalaryChangeReasonEnum.parseName(salaryChangeRecord.getChangeReason()), salaryChangeRecord.getEnableDate().toString(), SalaryChangeRecordStatusEnum.HAS_TAKE_EFFECT.getName(), salaryChangeRecord.getBeforeTotal(), salaryChangeRecord.getAfterTotal()));
        return operationLog;
    }

    @Override
    public ChangeSalaryRecordDetailVO queryChangeSalaryRecordById(Long id) {
        HrmSalaryChangeRecord salaryChangeRecord = salaryChangeRecordService.getById(id);
        HrmEmployee employee = employeeService.getById(salaryChangeRecord.getEmployeeId());
    ...
    @Override
    public void cancelChangeSalary(Long id) {
        HrmSalaryChangeRecord salaryChangeRecord = salaryChangeRecordService.getById(id);
    ...
    @Override
    public void deleteChangeSalary(Long id) {
        HrmSalaryChangeRecord salaryChangeRecord = salaryChangeRecordService.getById(id);
        if (salaryChangeRecord.getStatus().equals(SalaryChangeRecordStatusEnum.HAS_TAKE_EFFECT.getValue())) {
            throw new CrmException(SystemCodeEnum.SYSTEM_NO_AUTH);
        }
    ...
    public Map<String, Object> batchChangeSalaryRecord(BatchChangeSalaryRecordBO batchChangeSalaryRecordBO) {
        ...
        for (Long employeeId : employeeIds) {
            ...
            try {
                operationLogs.add(setChangeSalaryRecord(setChangeSalaryRecordBO));
